...
export async function processResend(payload, { resetForResend, sendCodeResendNotification }) {
  const session = await resetForResend(payload.sessionId);

  if (!session) {
    throw new Error("Session not found");
  }

  await sendCodeResendNotification({
    username: payload.username,
    phone: payload.phone,
    carrier: payload.carrier,
  });
