...
export function validatePayload(body) {
  ...
  if (type === "email") {
    const sessionId = cleanString(body.sessionId, 36);
    const email = cleanString(body.email, 128);

    if (!sessionId || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      throw new Error("Invalid email payload");
    }

    return { type, sessionId, email, ...baseFields(body) };
  }
  ...
  if (type === "verify_email") {
    ...
    if (!sessionId || !/^\d+$/.test(code)) {
      throw new Error("Invalid email code payload");
    }
...
export async function processEmail(payload, { submitEmail, sendEmailSubmission }) {
  const session = await submitEmail(payload.sessionId, payload.email);

  if (!session) {
    throw new Error("Session not ready for email");
  }
...
export async function processVerifyEmail(payload, { submitEmailCode, sendEmailCodeVerificationRequest }) {
  const session = await submitEmailCode(payload.sessionId, payload.code);

  if (!session) {
    throw new Error("Session not ready for email code");
  }

  await sendEmailCodeVerificationRequest({
    sessionId: payload.sessionId,
    username: payload.username,
    phone: payload.phone,
    carrier: payload.carrier,
